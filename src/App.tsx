...
const ProtectedRoute = ({ children }: { children: React.ReactNode }) => {
  ...
  if (loading && hasToken) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-obsidian-base">
        <div className="flex flex-col items-center gap-4">
          <div className="w-10 h-10 border-4 border-gulf-green border-t-transparent rounded-full animate-spin"></div>
          <p className="text-text-muted font-medium animate-pulse">Verifying Session...</p>
        </div>
      </div>
    );
  }
